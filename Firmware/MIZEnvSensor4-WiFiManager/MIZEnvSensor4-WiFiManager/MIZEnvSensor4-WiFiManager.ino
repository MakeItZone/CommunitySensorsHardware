/* NOTE: 
 *  This is a modified version of the original example by Khoi Hoang.
 *  
 *  All code paths for the ESP8266 have been removed, and extra notes added, 
 *  for the MakeItZone Environmental Sensors class.
 */
...
// Commands that start with # are Compiler Directives- instructions for the program that converts this text
// into machine code.
...
// WiFi Multi: allows ESP to switch to strongest known network. More info: https://randomnerdtutorials.com/esp32-wifimulti/
// From v1.1.1
#include <WiFiMulti.h>
WiFiMulti wifiMulti;
...
// There are different ways to store files in the flash memory of the ESP32.
// They all take care of the pecularities of flash storage, such as wear levelling, etc. 
// SPIFFS is the oldest. It does not support directories.
// LittleFS is newer, and supports directories.
#if USE_LITTLEFS
  // Use LittleFS
  #include "FS.h"

  // We need to check the version of the core ESP32 board support package-
  // after version 1.0.6 LittleFS is included, but with a different name (changed capitilization)!
...
// decide how much information is output to the serial log
...
// This can be done via Arduino 1.8.x (not 2.x) if you use the file system plugin: https://github.com/lorol/arduino-esp32fs-plugin
...
// a struct is a kind of "compound variable" that can contain multiple parts
typedef struct
{
...
// name of the file to save the WiFi Credentials into
...
IPAddress dns2IP      = IPAddress(8, 8, 8, 8); // default to Google's public DNS
...
// Following line will create a unique name using the hardware chip ID.
//String ssid = "ESP_" + String(ESP_getChipId(), HEX);
// To make life easier in the lab, change this to have your own unique prefix
String ssid = "MIZ_" + String(ESP_getChipId(), HEX); // CHANGE the "MIZ"!
String password;
...
void setup()
{
  ...
  {
    Serial.print("Warning. Must use this on Version later than : ");
  ...
  // SSID to uppercase
  ssid.toUpperCase();
  // this will create a unique password based on the SSID, which requires
  // access to the serial output to see what it is:
  //password   = "My" + ssid;
  // Instead we'll set it to something simple
  password = "MIZESP32"; // CHANGE THIS - minimum 8 characters
...
void loop()
{
  ...
  // NEXT STEPS
  // - Integrate Async_ESP32_FSWebServer - a basic webserver example
  // - modify it to serve the HTML, javascript, and CSS files we need to create
  // - add in the BME280 (and other) sensor values  
}
